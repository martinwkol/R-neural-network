...
public = list(
  ...
  initialize = function(layers, activationfct = "ReLU", dActivationfct = NULL,
    ...
    stopifnot("layers needs to be at least of length 3" = length(layers) >= 3)
    stopifnot("every layer must be greater than 0" = all(layers > 0))
...
nn$weights
nn$bias
nn$plot(image.save = FALSE, image.filename = ".test.png", image.type = "png")
